...
// Model for Personnel entity
    models.Personnel = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/PersonnelSubsystemDB/webresources/entities.personnel/",
        idAttribute: 'userId',
        defaults: {
            middleName: "",
            lastName: "",
        ...
        sync: function(method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/PersonnelSubsystemDB/webresources/entities.personnel/';
            }
    ...
    // Collection class for Personnel entities
    models.PersonnelCollection = Backbone.Collection.extend({
        model: models.Personnel,
        url: "http://localhost:8080/PersonnelSubsystemDB/webresources/entities.personnel/",
        sync: function(method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function(jqXHR, textStatus, errorThrown) {
...
$(function() {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function() {
            this.collection = new models.PersonnelCollection();
            var self = this;
            this.collection.fetch({
                success: function() {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-personnel-list-itemis template identifier for item
                        templateName: '#tpl-personnel-list-item'
                    });
        ...
        details: function(id) {
            if (this.collection) {
                this.personnel = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.personnel,
                    // tpl-personnel-details is template identifier for chosen model element
                    templateName: '#tpl-personnel-details',
        ...
        create: function() {
            ...
            var self = this;
            var dataModel = new models.Personnel();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-personnel-details is a template identifier for chosen model element
                templateName: '#tpl-personnel-details',
        ...
        getData: function() {
            return {
                ...
                trainingCertificateName: $('#trainingCertificateName').val(),
                postalCode: $('#postalCode').val(),
                mobilePhone: $('#mobilePhone').val(),
                admin: $('#admin').val(),
                numberYearsExperience: $('#numberYearsExperience').val(),
                ePhone: $('#ePhone').val(),
                eStates: $('#eStates').val(),
